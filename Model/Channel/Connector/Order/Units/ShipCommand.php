...
namespace M2E\Kaufland\Model\Channel\Connector\Order\Units;

class ShipCommand implements \M2E\Core\Model\Connector\CommandInterface
{
    private \M2E\Kaufland\Model\Account $account;
    /** @var \M2E\Kaufland\Model\Channel\Order\Units\Ship\Unit[] */
    private array $units;

    /**
     * @param \M2E\Kaufland\Model\Channel\Order\Units\Ship\Unit[] $units
     */
    ...
    public function parseResponse(\M2E\Core\Model\Connector\Response $response): object
    {
        ...
        foreach ($responseData['orders'] as $data) {
            ...
            foreach ($data['messages'] as $messageData) {
                $errors[] = new \M2E\Kaufland\Model\Channel\Order\Units\Ship\Error(
                    $data['order_unit_id'],
                    $messageData['text'],
                );
            }
        ...
        return new \M2E\Kaufland\Model\Channel\Connector\Order\Units\Ship\Response($errors);
    }
}
