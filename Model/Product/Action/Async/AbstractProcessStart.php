...
abstract class AbstractProcessStart
{
    ...
    private \M2E\Kaufland\Model\Product\Action\Async\Processing\InitiatorFactory $processingInitiatorFactory;
    ...
    public function initialize(
        ...
        \M2E\Kaufland\Model\Product\Action\Async\Processing\InitiatorFactory $processingInitiatorFactory,
        ...
        foreach ($this->getActionValidator()->getMessages() as $validatorMessage) {
            $this->addActionLogMessage(
                \M2E\Core\Model\Response\Message::create(
                    $validatorMessage->getText(),
                    $validatorMessage->getType()
                ),
            );
        }
    ...
    private function getProcessingParams(): \M2E\Kaufland\Model\Product\Action\Async\Processing\Params
    {
        $actionLogger = $this->getActionLogger();

        return new \M2E\Kaufland\Model\Product\Action\Async\Processing\Params(
