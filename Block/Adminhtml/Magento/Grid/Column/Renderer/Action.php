...
class Action extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\Action
{
    ...
    protected function _prepareLayout()
    {
        $this->js->add(
            <<<JS
    window.KauflandVarienGridAction = {
...
JS
    ...
    public function render(\Magento\Framework\DataObject $row): string
    {
        ...
        if (is_callable($actions)) {
            $actions = call_user_func($actions, $row);
        }
...
onchange="KauflandVarienGridAction.execute(this, '{$itemId}');">
    <option value=""></option>
...
HTML;
    ...
    protected function _toLinkHtml($action, \Magento\Framework\DataObject $row)
    {
        ...
        $action['onclick'] = "KauflandVarienGridAction.execute(this, '$itemId')";
