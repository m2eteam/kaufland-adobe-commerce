define([
    ...
    Order = Class.create(Common, {
        ...
        createHelpTitleHtml: function (rowId) {
            ...
            var closeHtml = '<a href="javascript:void(0);" onclick="OrderObj.hideOrderHelp(' + rowId + ');" title="' + Kaufland.translator.translate('Close') + '"><span class="hl_close icon-close"></span></a>';
        ...
        createHelpActionHtml: function (action) {
            ...
            } else if (action.type == Kaufland.php.constant('M2E_Kaufland_Model_Log_AbstractModel::TYPE_INFO')) {
            ...
            var type = '<span style="color: green;">' + Kaufland.translator.translate('Success') + '</span>';
            if (action.type == Kaufland.php.constant('M2E_Kaufland_Model_Log_AbstractModel::TYPE_INFO')) {
                type = '<span style="color: blue;">' + Kaufland.translator.translate('Info') + '</span>';
            } else if (action.type == Kaufland.php.constant('M2E_Kaufland_Model_Log_AbstractModel::TYPE_WARNING')) {
                type = '<span style="color: orange;">' + Kaufland.translator.translate('Warning') + '</span>';
            } else if (action.type == Kaufland.php.constant('M2E_Kaufland_Model_Log_AbstractModel::TYPE_ERROR')) {
                type = '<span style="color: red;">' + Kaufland.translator.translate('Error') + '</span>';
            }
        ...
        createHelpViewAllLogHtml: function (rowId, gridId) {
            let url = Kaufland.url.get('kaufland_log_order/index', {id: rowId});

            return '<div class="hl_footer"><a target="_blank" href="' + url + '">' + Kaufland.translator.translate('View Full Order Log') + '</a></div>';
        },
