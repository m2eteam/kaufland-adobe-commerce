    ...
    window.ListingViewAction = Class.create(Action, {
        ...
        sendPartsOfProducts: function (parts, totalPartsCount, url, requestParams) {
            ...
            if (parts.length == 0) {
                ...
                self.progressBarObj.setStatus(Kaufland.translator.translate('task_completed_message'));
                ...
                if (combineResult == 'error') {
                    ...
                    var message = Kaufland.translator.translate('task_completed_error_message');
                    message = message.replace('%task_title%', self.progressBarObj.getTitle());
                    message = message.replace('%url%', Kaufland.url.get('logViewUrl'));
                    ...
                    new Ajax.Request(Kaufland.url.get('getErrorsSummary') + 'action_ids/' + actionIds + '/', {
                        method: 'get',
                    ...
                    var message = Kaufland.translator.translate('task_completed_warning_message');
                    message = message.replace('%task_title%', self.progressBarObj.getTitle());
                    message = message.replace('%url%', Kaufland.url.get('logViewUrl'));

                    self.messageObj.addWarning(message);
                } else {
                    var message = Kaufland.translator.translate('task_completed_success_message');
                    message = message.replace('%task_title%', self.progressBarObj.getTitle());
            ...
            self.progressBarObj.setStatus(
                    str_replace(
                            '%product_title%',
                            partExecuteString,
                            Kaufland.translator.translate('sending_data_message')
                    )
            );
        ...
        previewItemsAction: function () {
            ...
            this.openWindow(
                    Kaufland.url.get('previewItems') + 'productIds/' + implode(',', orderedSelectedProductsArray)
