define(function () {
    ...
    window.KauflandListingPreviewItems = Class.create(Common, {

        initialize: function () {
            var self = this;

            jQuery('.price_container button').click(function () {
                self.alert(Kaufland.translator.translate('This is Item Preview Mode'));
            });
        ...
        getVariation: function (chosenOptions) {
            ...
            Kaufland.formData.variations.variations.forEach(function (variation) {
        ...
        onChangeVariationSelect: function (currentSelect) {
            ...
            if (choseSelects === $$('select').length - 1) {
                ...
                $$("select[value=]")[0].select('option[value!=]').each(function (option) {
                    ...
                    variation = KauflandListingPreviewItemsObj.getVariation(otherSelectsValues);
            ...
            if (Kaufland.formData.images.variations &&
                    Kaufland.formData.images.variations.specific === currentSelect.name && currentSelect.value) {
                $('product_image').src = Kaufland.formData.images.variations.images[currentSelect.value][0];
            }
        },
        ...
        initVariations: function () {
            $$('select').each(function (currentSelect) {
                if (currentSelect.select('option[value!=]').length === 1) {
                    ...
                    KauflandListingPreviewItemsObj.onChangeVariationSelect(currentSelect);
                }
            });
        }
    });
